...
class PoseDetector(TransformHelper, Draw, OpticalFlow):
    ...
    def _detect_and_get_pose(self, frame: np.ndarray, useflow=False,
        ...
        useflow:
            If True, optical flow else APE will be used
        outlier_method:
            The method to remove outliers from optical flow if
            optical flow used
        out:
            The window shown that displays the optical flow tracking

        Returns:
        Pose and points of the AprilGroup overlayed on the dodecahedron
        object, the 3D drawing of the dodecahedron in a seperate window,
        and the flow of the dodecahedron if optical flow is used.
        """
    ...
    def overlay_camera(self, useflow, outlier_method) -> None:
        ...
        Returns:
        Three video captured windows, one displays the object and the points
        returned from the pose, overlaid over the object, one displays the 3D
        drawing of the object pose and one displays the flow using optical flow tracking.
        """
        ...
        if success:
            ...
            self._detect_and_get_pose(frame, useflow=useflow,
                                      outlier_method=outlier_method, out=out)
